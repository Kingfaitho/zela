...
export default function ReferralSystem() {
  ...
  const userId = user?.email?.address || user?.phone?.number || "ZELA";
  const referralCode = userId.slice(0, 8).toUpperCase().replace("@", "");
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(0,212,170,0.2)", borderRadius: 16, padding: 20, marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        <span style={{ fontSize: 20 }}>🎁</span>
        <p style={{ fontWeight: 700, margin: 0, fontSize: 15 }}>Refer and Earn</p>
        <span style={{ color: "#00d4aa", fontSize: 11, marginLeft: "auto", background: "rgba(0,212,170,0.1)", padding: "2px 8px", borderRadius: 20 }}>$1 USDC per referral</span>
      </div>
      ...
      <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 10, padding: "10px 14px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", wordBreak: "break-all", flex: 1, marginRight: 8 }}>{referralLink}</span>
        <button onClick={copyLink} style={{ background: copied ? "#00d4aa" : "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, color: "white", fontSize: 12, padding: "6px 12px", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
          {copied ? "Copied!" : "Copy"}
        </button>
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={shareWhatsApp} style={{ flex: 1, padding: "12px", background: "#25D366", border: "none", borderRadius: 10, color: "white", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>
          Share on WhatsApp
        </button>
        <button onClick={shareTwitter} style={{ flex: 1, padding: "12px", background: "#1DA1F2", border: "none", borderRadius: 10, color: "white", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>
          Share on X
        </button>
      </div>
